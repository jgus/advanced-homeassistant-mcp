    ...
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        runScript = name: body: pkgs.writeShellApplication {
          inherit name;
          runtimeInputs = [ pkgs.bun pkgs.nodejs_latest ];
          text = body;
        };
        ...
        updateNpmDepsHashApp = pkgs.writeShellApplication {
          ...
          text = ''
            ...
            cd "$repo_root"
            NPM_FETCHER_VERSION=2 prefetch-npm-deps package-lock.json > npm-deps.hash
        ...
        stdioServerPkg = pkgs.buildNpmPackage {
          ...
          installPhase = ''
            ...
            # Custom installPhase replaces npmInstallHook, so prune devDeps here. --legacy-peer-deps mirrors npmFlags: the advisory fastmcp↔jose peer breaks `npm prune` re-resolution too, and npmFlags only reaches the nixpkgs hooks, not this literal command.
            npm prune --omit=dev --legacy-peer-deps --offline --no-audit --no-fund --no-update-notifier
            ...
            #!${pkgs.runtimeShell}
            exec ${pkgs.nodejs_latest}/bin/node $out/lib/ha-mcp-stdio-server/dist/stdio-server.mjs "\$@"
            EOF
